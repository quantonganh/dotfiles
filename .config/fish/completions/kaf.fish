...
function __kaf_perform_completion
    __kaf_debug "Starting __kaf_perform_completion"
...
# - Obtain the completions and store them in the global __kaf_comp_results
# - Return false if file completion should be performed
function __kaf_prepare_completions
    __kaf_debug ""
    __kaf_debug "========= starting completion logic =========="

    # Start fresh
    set --erase __kaf_comp_results

    set -l results (__kaf_perform_completion)
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __kaf_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__kaf_comp_results)
        set --global __kaf_comp_results $completions
        __kaf_debug "Filtered completions are: $__kaf_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__kaf_comp_results)
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__kaf_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                set --global __kaf_comp_results $split[1] $split[1].
                __kaf_debug "Completions are now: $__kaf_comp_results"
            end
        end
...
# The call to __kaf_prepare_completions will setup __kaf_comp_results
